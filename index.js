...
case 'ytdl':
if (isBanned) return reply(mess.only.benned)        
if (args.length === 0) return reply(`Example : ${prefix + command} dj tutu 30 detik`)
var srch = args.join('')
res = await yts(`${srch}`).catch(e => {
reply('_[ ! ] Error Nama Lagu Yang Anda Masukan Tidak Ada_')
})
let ini_gambarlagu = res.all[0].image
let ini_textlagu = `*Youtube Play*
...
frnky.send2ButtonImg(from, ini_textlagu, ini_gambarlagu, 'Silahkan Di Pilih Mau Tipe Yang Mana!' , '🎵AUDIO', `${prefix}buttonmusic ${res.all[0].url}`, '🎥VIDEO', `${prefix}buttonvideo ${res.all[0].url}`, )
break
case 'buttonmusic':
...
case 'sourcecode': 
       case 'sc': 
       case 'src':
             reply(`Bot ini menggunakan sc : https://github.com/Franky404/Whatsapp-bot`)
              break
case 'attp':
...
case 'nhentaipdf':
                    ...
                    await frnky.sendMessage(from, ini_buffer, document, { quoted: Kyz, mimetype: Mimetype.pdf, filename: `${henid}.pdf` })
                    break
